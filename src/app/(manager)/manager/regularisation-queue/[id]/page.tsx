...
/**
 * M-07 — Regularisation Detail (Manager)
 * Visual reference: prototype/manager/regularisation-detail.html
 *
 * - Full regularisation detail
 * - Original record vs proposed correction side-by-side
 * - Approve/Reject actions if Pending + routedTo = Manager
 */
    ...
    return (
      <div className="p-8">
        <div role="alert" className="bg-crimsonbg border border-crimson/40 rounded-xl p-6 text-crimson text-sm">
          Failed to load regularisation: {errMsg}
        </div>
        <Link href="/manager/regularisation-queue" className="mt-4 inline-flex text-sm text-forest hover:underline">
          ← Back to queue
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-8">
      ...
      <nav aria-label="Breadcrumb" className="flex items-center gap-1 text-xs text-slate mb-6">
        <Link href="/manager/regularisation-queue" className="hover:text-forest transition-colors">
          Reg Queue
        </Link>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        ...
        {canDecide && (
          <RegularisationApprovalActions
            ...
            version={reg.version}
            onDecision={() => router.push('/manager/regularisation-queue')}
          />
        )}
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-sage/30 p-6">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <div className="font-semibold text-charcoal">{reg.ageDaysAtSubmit} days</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="font-semibold text-charcoal">{reg.approverName ?? '—'}</div>
            </div>
          </div>
        </div>
            ...
            <div className="bg-offwhite rounded-lg p-4">
              ...
              <div className="space-y-3 text-sm">
                ...
                {reg.correctedRecordId && (
                  <div>
                    <div className="text-xs text-slate mb-0.5">Corrected Record ID</div>
