...
/**
 * A-08 — Leave Configuration (Admin)
 * Visual reference: prototype/admin/leave-config.html
 *
 * - Per-type carry-forward cap + maxDaysPerEvent edits
 * - Per-employment-type quota matrix edits
 * - Inline edit + save per row with optimistic UI
 */
...
function QuotaCell({
    ...
    return (
      <div className="flex items-center gap-1 justify-center">
        ...
        <button
          ...
          aria-label="Save"
          className="text-richgreen hover:text-forest"
        >
  ...
  return (
    <button
      ...
      className="font-semibold text-charcoal hover:text-forest hover:underline transition-colors"
...
// ── Type config row (carry-forward + max event) ───────────────────────────────
...
export default function AdminLeaveConfigPage() {
  ...
  return (
    <div className="p-6 md:p-8">
      {/* Breadcrumb */}
      <div className="mb-6">
        <h1 className="font-heading text-xl font-semibold text-charcoal">Leave Configuration</h1>
        <div className="text-xs text-slate flex items-center gap-1 mt-0.5">
          <Link href="/admin/leave-queue" className="hover:text-forest transition-colors">Leave</Link>
          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
          <span className="text-charcoal font-medium">Configuration</span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          {/* Section 1: Leave Quotas (per employment type) */}
          ...
          {/* Section 2: Carry-forward caps + event maxima */}
          <div className="bg-white rounded-xl shadow-sm border border-sage/30 overflow-hidden">
            <div className="px-6 py-4 border-b border-sage/20">
