...
function formatDate(iso: string): string {
  try { return new Date(iso + 'T00:00:00').toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }); }
  catch { return iso; }
}
...
export default function PayrollLeaveDetailPage() {
  ...
  if (error || !request) return (
    <div className="p-8">
      ...
      <Link href="/payroll/leave" className="text-sm text-forest hover:underline mt-4 block">Back to My Leave</Link>
    </div>
  );
    ...
    <div className="p-6 md:p-8 max-w-3xl">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-sage/30 overflow-hidden mb-5">
        ...
        <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5">
          <div><div className="text-xs font-semibold text-slate uppercase tracking-wide mb-1">Type</div><div className="text-sm font-semibold text-charcoal">{request.type} Leave</div></div>
          <div><div className="text-xs font-semibold text-slate uppercase tracking-wide mb-1">Duration</div><div className="text-sm font-semibold text-charcoal">{request.fromDate === request.toDate ? formatDate(request.fromDate) : `${formatDate(request.fromDate)} – ${formatDate(request.toDate)}`}</div></div>
          <div className="sm:col-span-2"><div className="text-xs font-semibold text-slate uppercase tracking-wide mb-1">Reason</div><div className="text-sm text-charcoal">{request.reason}</div></div>
          {request.decisionNote && <div className="sm:col-span-2"><div className="text-xs font-semibold text-slate uppercase tracking-wide mb-1">Decision Note</div><div className="text-sm text-charcoal">{request.decisionNote}</div></div>}
        </div>
      </div>
      {canCancel && (
        <div className="bg-white rounded-xl shadow-sm border border-sage/30 p-6">
          <p className="text-sm text-slate mb-4">{beforeStart ? 'Full balance restored (BL-019).' : 'Partial restore only (BL-020).'}</p>
          <Button variant="destructive" onClick={() => setCancelOpen(true)}>Cancel Leave Request</Button>
        </div>
      )}
      {cancelOpen && <CancelLeaveModal isOpen={cancelOpen} onClose={() => setCancelOpen(false)} request={request} onConfirm={handleCancel} isLoading={cancelMutation.isPending} />}
    </div>
  );
}
