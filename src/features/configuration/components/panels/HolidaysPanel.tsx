...
/**
 * HolidaysPanel — inline version of /admin/holidays.
 * Renders the holiday calendar editor without page-level chrome.
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="font-heading text-base font-semibold text-charcoal">Holiday Calendar</h2>
        ...
        <div className="flex items-center gap-3">
          <div>
            <label htmlFor="panel-holiday-year" className="sr-only">Year</label>
            <input
              id="panel-holiday-year"
              type="number"
              value={year}
              min={2000}
              max={2999}
          ...
          {hasPendingEdits && (
            <Button variant="primary" size="sm" onClick={handleSaveAll} loading={replaceMutation.isPending}>
              Save {year} Calendar
            </Button>
          )}
        </div>
                ...
                {rows.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center text-sm text-slate py-10">
                      No holidays configured for {year}. Click &ldquo;Add Holiday&rdquo; to begin.
                    </td>
                  </tr>
                ) : (
                  rows
                          ...
                          <td className="px-4 py-3">
                            {row.editing ? (
                              ...
                              <span className="text-charcoal">
                                {row.name || <span className="italic text-slate">Unnamed</span>}
                              </span>
                            )}
                          </td>
