 ...
 *
 * Mirrors the prototype hero card markup exactly:
 *   - dot-grid overlay (nx-dotgrid)
 *   - two animated blobs (nx-b1 / nx-b2) for depth
 *   - sun SVG for morning / day
 *   - star field for night
 *   - time-of-day gradient driven by data-tod attribute + globals.css rules
 *
 * The demo dock (nx-tod-demo) from prototype is intentionally omitted (SRS §10).
 */
...
export function TimeOfDayHero({ firstName, subtitle, badge }: TimeOfDayHeroProps) {
  ...
  return (
    ...
    >
      ...
      {/* Animated blobs */}
      <div
        className="nx-b1 absolute -top-16 -left-12 w-[18rem] h-[18rem] rounded-full bg-emerald/40 blur-3xl pointer-events-none"
        aria-hidden="true"
      />
      <div
        className="nx-b2 absolute -bottom-20 -right-16 w-[20rem] h-[20rem] rounded-full bg-mint/25 blur-3xl pointer-events-none"
        aria-hidden="true"
      />
