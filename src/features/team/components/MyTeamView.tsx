...
/**
 * MyTeamView — the "My Team" page body, reused by:
 *   - Manager: /manager/team
 *   - Admin (when the admin is a line manager for any employee): /admin/team
 *
 * The visible content is identical; only the "View Profile" target URL and the
 * empty-state copy differ between the two surfaces — both are passed in as
 * props so this file stays free of role-specific URL logic.
 *
 * Visual reference: prototype/manager/my-team.html.
 * Two tabs (Current / Past) with client-side pagination over the bundled
 * `useTeam(meId)` response — see BL-022a.
 */
...
function CurrentMemberCard({ member, profileHref }: MemberCardProps) {
...
function PastMemberCard({ member, profileHref }: MemberCardProps) {
  const initials = getInitials(member.name);
  return (
...
export interface MyTeamViewProps {
  /** Builds the View-Profile URL for a member id. Manager → /manager/team/{id},
   *  Admin → /admin/employees/{id} (admins drop into the full editable
   *  profile rather than the read-only manager surface). */
  profileHrefBuilder: (memberId: number) => string;
  /** Copy shown on the empty-current-team state. Admin and manager surfaces
   *  point users in different directions for fixing this. */
  emptyHint?: string;
}

export function MyTeamView({
  profileHrefBuilder,
  emptyHint = 'Contact Admin to assign employees to your team.',
}: MyTeamViewProps) {
  ...
  return (
    <div>
      {/* Status chips — page title + subtitle live in the global TopBar
          (see PAGE_META entries for /manager/team and /admin/team), so we
          only render the count chips here. */}
      {(activeCount > 0 || onLeaveCount > 0 || onNoticeCount > 0) && (
        ...
        <div>
          {current.length === 0 ? (
            <div className="bg-white rounded-xl border border-sage/30 py-16 text-center">
              ...
              <p className="text-xs text-sage mt-1">{emptyHint}</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-4">
                {pageItems.map((member) => (
                  <CurrentMemberCard
                    key={member.id}
                    member={member}
                    profileHref={profileHrefBuilder(member.id)}
                  />
                ))}
              </div>
              <CursorPaginator
